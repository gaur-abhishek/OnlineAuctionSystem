...
function doLogin()
{
	...
	if ($userName == '') {
		...
		if (dbNumRows($result) == 1) {
			...
			$_SESSION['auction_user_id'] = $row['id'];
			$_SESSION['auction_is_admin'] = $row['is_admin'];
...
{
	if (isset($_SESSION['auction_user_id'])) {
		unset($_SESSION['auction_user_id']);
		session_unregister('auction_user_id');
	}
	if (isset($_SESSION['auction_is_admin'])) {
		unset($_SESSION['auction_is_admin']);
		session_unregister('auction_is_admin');
	}
...
function buildCategoryOptions($catId = 0)
{
	$sql = "SELECT id, cname
			FROM tbl_categories
			ORDER BY id";
	$result = dbQuery($sql) or die('Cannot get Category. ' . mysql_error());
	$list = '';
	while($row = dbFetchAssoc($result)) {
		extract($row);
		$list .= "<option value=".$id.">".$cname."</option>\r\n";
	}	
	return $list;
}
...
function buildBankOptions()
{
	$sql = "SELECT b.id, b.bname, c.city 
			...
			WHERE b.city = c.id
			ORDER BY b.id";
	$result = dbQuery($sql) or die('Cannot get Bank. ' . mysql_error());
	$list = '';
	while($row = dbFetchAssoc($result)) {
		extract($row);
		$list .= "<option value=".$id.">".$bname." - ( ".$city." )</option>\r\n";
	}	
	return $list;
}
...
function buildCityOptions()
{
	$sql = "SELECT id, city 
			FROM tbl_cities c
			ORDER BY id";
	$result = dbQuery($sql) or die('Cannot get City. ' . mysql_error());
	$list = '';
	while($row = dbFetchAssoc($result)) {
		extract($row);
		$list .= "<option value=".$id.">".$city."</option>\r\n";
	}	
	return $list;
}
