...
function _prepare_url_text($string){
	$match_char = '#[^-a-zA-Z0-9_ ]#'; // match the regex char to string 
	$string = ucwords($string); // convert to lower case
...
function create_final_url($category, $article_name, $article_id, $char) {
	$category = _prepare_url_text($category);
	$article_name = _prepare_url_text($article_name);
	$final_url = WEB_ROOT . $category . '/' . $article_name . '-'.$char .$article_id . '.html';
	return $final_url;
}// create_final_url
